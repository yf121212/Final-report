...
class racetrack(object):
    ...
    def __init__(self, num_turns, seed, max_len=8, min_len=3, max_angle=1 * math.pi/4, min_angle=-1 * math.pi/4):
        # Setting seed
        np.random.seed(seed)
    ...
    def generate(self, plot=False):
        ...
        for i in range(self.num_turns):
            prev_point = points[-1]
            prev_angle = angles[-1]
            new_point, new_angle, new_len = self.calc_new_point(prev_point, prev_angle)
        ...
        return racetrack, points[-1], lens, points, angles #angles of each turn
